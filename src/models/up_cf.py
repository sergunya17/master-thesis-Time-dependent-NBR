...
class UPCFRecommender(IRecommender):
    ...
    def fit(self, dataset: NBRDatasetBase):
        ...
        self._user_item_matrix_implicit = calculate_user_item_matrix(
            dataset.train_df, dataset.num_users, dataset.num_items,
        )
        self._user_item_matrix_implicit = preprocess_matrix(self._user_item_matrix_implicit, "binary")
...
class UPCFTimeRecommender(IRecommender):
    def __init__(
        self,
        q: int = 5,
        alpha: float = 0.25,
        ...
        self.topk_neighbors = topk_neighbors
        self.preprocessing = preprocessing
        self.time_recency = time_recency
    ...
    def fit(self, dataset: NBRDatasetBase):
        self._user_item_matrix = calculate_user_item_matrix(
            dataset.train_df, dataset.num_users, dataset.num_items, time_recency=self.time_recency
        )
        self._user_item_matrix = preprocess_matrix(self._user_item_matrix, self.preprocessing)
...
class UPCFTimeNextTsRecommender(IRecommenderNextTs):
    def __init__(
        self,
        q: int = 5,
        alpha: float = 0.25,
    ...
    def fit(self, dataset: NBRDatasetBase):
        ...
        self._user_item_matrix = calculate_user_item_matrix(
            ...
            dataset.num_users,
            dataset.num_items,
            time_recency=self.time_recency,
            use_next_basket_ts=False,
        )
        self._user_item_matrix = preprocess_matrix(self._user_item_matrix, self.preprocessing)
    ...
    def predict(self, user_ids, user_next_basket_ts: pd.DataFrame, topk=None):
        ...
        user_item_matrix = calculate_user_item_matrix(
            pred_df,
            self._train_num_users,
            self._train_num_items,
            time_recency=self.time_recency,
            use_next_basket_ts=True,
        )
        user_item_matrix = preprocess_matrix(user_item_matrix, self.preprocessing)
