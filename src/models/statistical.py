...
class GTopFreqRecommender(IRecommender):
    ...
    def predict(self, user_ids, topk=None):
        ...
        item_pop_topk[np.argsort(item_pop_topk, kind="stable")[:-topk]] = 0
...
class PTopFreqRecommender(IRecommender):
    def __init__(self, min_freq: int = 2, preprocessing: str = None) -> None:
        super().__init__()
    ...
    def fit(self, dataset: NBRDatasetBase):
        ...
        self._item_personal = self._user_item_matrix.multiply(
            self._user_item_matrix >= self.min_freq
        )
        self._item_personal = preprocess_matrix(self._item_personal, self.preprocessing)

        return self

    def predict(self, user_ids, topk=None):
        if topk is None:
            topk = self._item_personal.shape[1]

        item_scores = self._item_personal[user_ids].copy()
...
class GPTopFreqRecommender(IRecommender):
    def __init__(
        self,
        min_freq: int = 2,
