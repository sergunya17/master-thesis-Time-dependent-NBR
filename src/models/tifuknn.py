...
class TIFUKNNRecommender(IRecommender):
    ...
    def fit(self, dataset: NBRDatasetBase):
        user_basket_df = dataset.train_df.groupby("user_id", as_index=False).apply(
            self._calculate_basket_weight
        )
...
class TIFUKNNTimeDaysRecommender(TIFUKNNRecommender):
    ...
    def _calculate_basket_weight(self, df: pd.DataFrame):
        ...
        df["group_max_timestamp"] = max_timestamp - (
            df["group_num"] * pd.Timedelta(days=self.group_size_days)
        )

        df["local_days_diff"] = (df["group_max_timestamp"] - df["timestamp"]) / pd.Timedelta(
            days=1
        )
